package com.pwr.expertsystem.experimental

class RiskGroupsInterview {
    val sex = Question.RadioQuestion("Jaka jest płeć pacjenta", arrayOf("mężczyzna", "kobieta"))
    val age = Question.NumericalQuestion("Ile pacjent ma lat?")
    val smoking = Question.BooleanQuestion("Czy pacjent pali?")
    val drinking = Question.BooleanQuestion("Czy pacjent pije?")
    ...
    val medicines = Question.AutoFillQuestion(
...
class Condition<T>(
    ...
    get() = when (val answer = question.answerStatus) {
        ...
        is Answered -> Checked(result = evaluation(answer.value))
    }
}
...
fun main() {
    ...
    val riskGroupsRules = listOf(
        Rule(
            conditions = setOf(
                Condition("Pacjent ma co najmniej 40 lat", interview.age) { it >= 40 }
            ),
        ...
        Rule(
            conditions = setOf(
                Condition("Pacjent jest mężczyzną", interview.sex) { it == "mężczyzna" },
                Condition("Pacjent ma co najmniej 40 lat", interview.age) { it >= 40 },
                Condition("Pacjent pali wyroby tytoniowe", interview.smoking) { it },
                Condition("Pacjent pije alkohol", interview.drinking) { it }
            ),
        ...
        Rule(
            conditions = setOf(
                Condition("Pacjent ma co najmniej 60 lat", interview.age) { it >= 60 },
                Condition("Pacjent bierze NSLPZ lub leki przeciwkrzepliwe", interview.medicines) {
                    it.any { medicine ->
...
fun Rule.process() {
    ...
    if(conditions.all { it.conditionStatus == Checked(true) }){
        conclusion.invoke()
    }
}
...
fun Question<out Any>.ask(){
    ...
    when(this){
        is Question.BooleanQuestion -> answerStatus = Answered(result.toBoolean())
        is Question.RadioQuestion -> answerStatus = Answered(result)
        is Question.AutoFillQuestion -> answerStatus = Answered(listOf(result))
        is Question.NumericalQuestion -> answerStatus = Answered(result.toInt())
    }
}